%% the main UE experiment testing program in graph An
...
% Input the dataset
rawnet_trips = xlsread('Annetwork.xlsx','demand'); % Input the graph
rawnet_network = xlsread('Annetwork.xlsx','network');  % Input the graph
nodenum=416;
zonesnum=38;
...
for i_network=1:zonesnum
    ...
    for i_38=1:zonesnum
        if mod(i_38,5)==0
            ...
            tmp_count=mod(i_38,5);
        end
        if (rawnet_trips(1+(i_network-1)*10+ceil(i_38/5),2*tmp_count)~=0 && ~(isnan(rawnet_trips(1+(i_network-1)*10+ceil(i_38/5),2*tmp_count))))
            list_sinks{i_network,1}=[list_sinks{i_network,1},rawnet_trips(1+(i_network-1)*10+ceil(i_38/5),(tmp_count-1)*2+1)];
            list_demand{i_network,1}=[list_demand{i_network,1},rawnet_trips(1+(i_network-1)*10+ceil(i_38/5),2*tmp_count)];
        end
    end
end
%% the testing program
for i_times=1:repeat_times
    ...
    xlswrite('result_An.xlsx',currentresult3,'originalUE_RGAP',xlRange);
    %% the UE-ED(ours)
    currentresult3=zeros(10000,2);
    RGAP=1;
    ite=0;
    ...
    currentresult3(:,1)=cumsum(currentresult3(:,1));
    xlRange=sprintf("%c1",65+(i_times-1)*2);
    xlswrite('result_An.xlsx',currentresult3,'UE_ED_RGAP',xlRange);
end
